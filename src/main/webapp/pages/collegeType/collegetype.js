define(['text!pages/collegeType/collegetype.html','css!pages/collegeType/collegetype','pages/collegeType/meta','uuitree','uuigrid'],function(html) {
	var init = function(element){
		var treelistUrl = ctx+'/collegeType/list';
		var treedelUrl = ctx+'/collegeType/del/';
		var treesaveUrl = ctx+'/collegeType/save';
		
		var viewModel = {
				app:{},
				dt1 : new u.DataTable(metaCollegeType),
				dtnew : new u.DataTable(metaCollegeType),
				...
				event:{
					...
					addCollegeTypeClick:function(){
                        $('#code').removeAttr("readonly");
						$('#dialog_content_instit').find('.u-msg-title').html("<h4>新增学院类别</h4>");
						...
						viewModel.dtnew.setValue('student_num',0);	
						
			            window.md = u.dialog({id:'add_college',content:"#dialog_content_instit",hasCloseMenu:true});
					},
					editCollegeTypeClick:function(){
						$('#dialog_content_instit').find('.u-msg-title').html("<h4>编辑学院类别</h4>");
						viewModel.event.clearDt(viewModel.dtnew);
						var row = viewModel.dt1.getSelectedRows()[0];
						if(row){
							viewModel.dtnew.setSimpleData(viewModel.dt1.getSimpleData({type: 'select'}));
							$('#code').attr("readonly","readonly");
							window.md = u.dialog({id:'edit_college',content:"#dialog_content_instit",hasCloseMenu:true});
						}else{
							u.messageDialog({msg:'请选择要编辑的数据！',title:'操作提示',btnText:'确定'});
						}
					},
					delCollegeTypeClick:function(){
						var row = viewModel.dt1.getSelectedRows()[0];
						if(row){
					...
					cancelCollegeTypeClick:function(){
							md.close();
					},
				}
			};
